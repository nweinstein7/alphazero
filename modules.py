...
class TrainableModel(AbstractModel):
    ...
    def __consume_generator(self,
        ...
        for data in data_generator:
            print("Consuming one piece of data")
            if fit: self.fit_on_batch(data)
            print("Training")
            self.train(fit)
            print("Done training")

            data, data_pred = self._AbstractModel__consume_batch(
                data, mask_missing=mask_missing)
            print("Done consuming batch")
    ...
    def score(self, preds, targets):
        ...
        score = accuracy_score(np.argmax(preds['target'], axis=1),
                               targets['target'])
        base_score = accuracy_score(np.argmax(preds['target'], axis=1),
